...
class DataMaker(object):
    ...
    def generate_inputs(self, data, max_seq_len, ent2id, data_type="train"):
        """生成喂入模型的数据

        Args:
            data (list): json格式的数据[{'text':'','entity_list':[(start,end,ent_type),()]}]
...
class MetricsCalculator(object):
    ...
    def get_sample_f1(y_pred, y_true):
        ...
        return 2 * torch.sum(y_true * y_pred) / torch.sum(y_true + y_pred + 1e-5)
...
class GlobalPointer(nn.Module):
    def __init__(self, encoder, ent_type_size, inner_dim, RoPE=False):
        super().__init__()
        self.device = None
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        outputs = self.dense(last_hidden_state)  # shape (batch_size, seq_len, ent_type_size*inner_dim*2)
        outputs = torch.split(outputs, self.inner_dim * 2, dim=-1)
        outputs = torch.stack(outputs, dim=-2)  # shape (batch_size, seq_len, ent_type_size, inner_dim*2)
