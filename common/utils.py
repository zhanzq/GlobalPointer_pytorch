...
class Preprocessor(object):
    ...
    def get_ent2token_spans(self, text, entity_list):
        ...
        for ent_span in entity_list:
            ...
            # 寻找ent的token_span
            token_start_idx = [i for i, v in enumerate(text2tokens) if v == ent2token[0]]
            token_end_idx = [i for i, v in enumerate(text2tokens) if v == ent2token[-1]]
            
            token_start_index = list(filter(lambda x: token2char_span_mapping[x][0] == ent_span[0], token_start_idx))
            # token2char_span_mapping[x][-1]-1 减1是因为原始的char_span是闭区间，而token2char_span是开区间
            token_end_index = list(filter(lambda x: token2char_span_mapping[x][-1]-1 == ent_span[1], token_end_idx))
